...
## Round 2 (same session) — /faq/ + /giveaway/ built · UNCOMMITTED as of writing (5 files)
Closes 2 of the 4 remaining legacy-URL orphans. `/es/` ×2 deliberately left alone (separate WordPress build). Linux build: **83 pages, 0 errors**; sitemap 80 URLs (404, thank-you, giveaway excluded).
- **`src/pages/faq.astro` (NEW, indexable)** — full legacy WP `/faq/` content ported: 3 groups / 19 Q&As (Personal Injury 6, Auto Accidents 7, Slip and Fall 6) + each group's phone CTA outro. Careers/privacy shell + in-page section nav + FAQPage JSON-LD (19 Questions, text flattened from the same data). Two edits vs. the WP original: (1) "Partly to blame" answer → Eric's approved comparative-negligence paragraph (WP said "less than 50%" — wrong, Eric's rule is 50% or less); (2) "property authorities" → "proper authorities" ×2. **⚠ LEGAL: this copy is the firm's old published FAQ text, NOT in Eric's 2026 review.** Flag for him: "Punitive damages are *required* by courts if malicious intent" (they're permitted/awarded, not required — NRS 42.005 oppression/fraud/malice); "one month to one year" case length. Unblocks the 2 legacy sources `/faq/personal-injury-faq/` + `/video-faq/` (rules already existed, target now 200).
- **`src/pages/giveaway.astro` (NEW, `noindex` via `active = false`)** — rules TEMPLATE only, no giveaway content. 10 section headings mirrored from the 2024 Chayanne page (Eligibility → Privacy), each "To be announced."; hero says no giveaway running + FB/IG buttons (URLs from `firm.ts`). To run a giveaway: fill `giveaway` + `rules[].text`, set `active = true`, remove `/giveaway` from the sitemap filter.
- **`public/_redirects`** +2 lines: dated comment + `/chayannegiveaway/ → /giveaway/ 301`. 342 → **343 rules**. (Header comment still says 292 — stale, cosmetic.)
- **`astro.config.mjs`** — sitemap filter now also excludes `/giveaway` (noindex page shouldn't be in the sitemap). Verified `/blog/las-vegas-motorcycle-giveaway/` still IN the sitemap (hyphen, not slash, before "giveaway").
- NOT done (ask H.K.): footer/nav link to `/faq/` — page is currently reachable only via the 2 redirects (orphan, same situation careers was in before `a414432`). One-line fix in `Footer.astro` `legalLinks`.
- Redirect audit after deploy: expect **347 / 349** (was 345; `/faq/`-family + `/chayannegiveaway/` now resolve). Remaining 2 = the `/es/` pair.
